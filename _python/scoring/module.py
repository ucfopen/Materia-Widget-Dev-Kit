...
class ScoreModule(ABC):
    ...
    def __init__(self, play: LogPlay):
        ...
        # TODO: figure out how to parse JSON files to account for these
        self.qset = play.qset.data
        self.questions = []
            ...
            if game_time < last_time and game_time != -1:
                if self.log_problems:
                    ...
                    error_log = Log(
                        ...
                        value=str(last_time),
                        game_time=game_time,
                        # TODO: let the target time zone be configurable somehow?
        ...
        for log in self.logs:
            if log.log_type == Log.LogType.WIDGET_END:
                ...
                try:
                    ...
                    print("Invalid participation log value - must be a number.")
    ...
    # parse the json representing this qset for a given question by id
    ...
    def details_for_question_answered(self, log) -> dict:
        ...
        return {
            ...
            # this is a little stupid - django's "gettext_lazy" would handle the
            #  translation of this numeric value  back into a string for us, but
            #  we have to do it ourselves here
            "type": log.log_type_string,
...
class EmptyScoreModule(ScoreModule):
    def __str__(self):
        return "an empty score module, go fuck yourself"
