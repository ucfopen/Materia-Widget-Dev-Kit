...
class ScoreModuleFactory:
    ...
    def create_score_module(
        ...
        try:
            ScoreClass = cls._load_score_class(instance)
            if not ScoreClass:
    ...
    @staticmethod
    def _load_score_class(instance: WidgetInstance) -> Optional[Type[ScoreModule]]:
        try:
            code = Path("/score_module/score_module.py").read_text()
